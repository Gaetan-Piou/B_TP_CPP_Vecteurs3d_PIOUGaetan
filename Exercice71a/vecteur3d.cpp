...
float CVecteur3d::calculNormeVecteur() {
	//BUT: Calculer la norme du vecteur
	//ENTREE: Aucune
	//SORTIE: La norme du vecteur

	float norme = sqrtf(powf(this->getX(),2) + powf(this->getY(),2) + powf(this->getZ(),2));
	return norme;
}


CVecteur3d CVecteur3d::normaxVecteur(CVecteur3d vect) {
	//BUT: Verifier lequel des deux vecteur a la plus grande norme
	//ENTREE: Un autre vecteur
	//SORTIE: Le vecteur avec la plus grande norme

	float norm1 = this->calculNormeVecteur();
	float norm2 = vect.calculNormeVecteur();
